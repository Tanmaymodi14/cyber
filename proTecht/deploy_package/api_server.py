...
def get_db():
    ...
    if db is None:
        ...
        if db_url:
            # Use PostgreSQL adapter when DB_URL is provided
            from db_postgres import ProTechtDatabasePG  # type: ignore
            db = ProTechtDatabasePG(db_url)
        else:
            db = ProTechtDatabase()
    return db
...
_aws_data_cache = {}
ai_agent = None
if ENABLE_AI and AIAnalysisAgent is not None:
    try:
...
def get_aws_evidence():
    try:
        data = get_db().load_aws_data_from_db()
...
def recollect_aws(profile: Union[str, None] = None, regions: Union[str, None] = None, authorization: Union[str, None] = Header(default=None)):
    ...
    try:
        ...
        get_db().load_aws_data(collected)
...
def upload_fedramp_template(file: UploadFile = File(...), template_name: str = Form("FedRAMP Template"), authorization: Union[str, None] = Header(default=None)):
    ...
    try:
        ...
        try:
            success = get_db().store_fedramp_template(
...
def upload_policy(file: UploadFile = File(...), policy_name: str = Form("Uploaded Policy"), authorization: Union[str, None] = Header(default=None)):
    ...
    try:
        ...
        # Store in database
        get_db().store_policy(policy_data)
...
def reanalyze_policy(policy_id: str, authorization: Union[str, None] = Header(default=None)):
    ...
    try:
        ...
        # Update in database
        get_db().store_policy(updated_policy_data)
