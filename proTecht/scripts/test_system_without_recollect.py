#!/usr/bin/env python3
"""
System Test Without Recollect
Tests all functionality except AWS recollect to avoid clearing test data
"""
...
def test_controls_evaluation():
    ...
    try:
        ...
        # Check if we have no_data status
        if isinstance(data, dict) and data.get('status') == 'no_data':
            print("⚠️ No AWS data available - this is expected if no real AWS data is collected")
            return True
...
def test_frontend_integration():
    ...
    try:
        ...
        if isinstance(data, dict) and data.get('status') == 'no_data':
            print("✅ Controls endpoint returns proper no_data status for frontend")
            return True
...
def test_real_data_flow():
    ...
    try:
        ...
        if has_real_data:
            ...
            print("   This is expected if no real AWS data is collected")
...
def test_aws_recollect_endpoint():
    """Test that AWS recollect endpoint exists and responds (without actually calling it)"""
    print("🔄 Testing AWS recollect endpoint availability...")
    
    base_url = "http://localhost:8000"
    
    try:
        # Test that the endpoint exists by checking if it responds to OPTIONS
        response = requests.options(f"{base_url}/api/evidence/aws/recollect")
        if response.status_code in [200, 405]:  # 405 is Method Not Allowed, which is fine for OPTIONS
            print("✅ AWS recollect endpoint is available")
            return True
        else:
            print(f"❌ AWS recollect endpoint failed: {response.status_code}")
            return False
    except Exception as e:
        print(f"❌ AWS recollect endpoint error: {e}")
        return False

def main():
    """Main function to run complete system test"""
    print("🚀 proTecht System Test (Without Recollect)")
    print("=" * 50)
    ...
    tests = [
        ...
        ("AWS Recollect Endpoint", test_aws_recollect_endpoint)
    ]
